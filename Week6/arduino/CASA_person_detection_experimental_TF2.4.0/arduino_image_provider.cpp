...
TfLiteStatus InitCamera(tflite::ErrorReporter* error_reporter) {
  ...
  // PATCH - SPI set to 8MHz mode 0 per https://www.arducam.com/docs/spi-cameras-for-arduino/hardware/arducam-shield-mini-2mp-plus/#8-spi-slave-interface
...
TfLiteStatus DecodeAndProcessImage(tflite::ErrorReporter* error_reporter,
  ...
  // Coded Units (MCUs), which are 16x8 blocks of pixels.
    ...
    TF_LITE_REPORT_ERROR(error_reporter, "jpeg_buffer %d ", jpeg_buffer);
    TF_LITE_REPORT_ERROR(error_reporter, "jpeg_length %d ", jpeg_length);

  JpegDec.decodeArray(jpeg_buffer, jpeg_length);

  Serial.println("in main loop");
    TF_LITE_REPORT_ERROR(error_reporter, "JpegDec.MCUWidth %d ", JpegDec.MCUWidth);
    TF_LITE_REPORT_ERROR(error_reporter, "JpegDec.MCUHeight %d ", JpegDec.MCUHeight);
